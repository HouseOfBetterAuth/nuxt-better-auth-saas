...
const shouldShowSidebar = computed(() => {
  const path = route.path
  return path.startsWith('/conversations') || path.startsWith('/content')
})
...
<template>
  <UDashboardGroup storage-key="dashboard-sidebar" storage="localStorage">
    ...
    >
      ...
      <template #default="{ collapsed }">
        ...
        <div v-else class="flex flex-col items-center gap-2 py-4">
          <UIcon name="i-lucide-message-circle" class="w-5 h-5 text-muted-500" />
          <UIcon name="i-lucide-file-text" class="w-5 h-5 text-muted-500" />
        </div>
      </template>
    ...
    <UDashboardPanel>
      <template #header>
        <UDashboardNavbar>
          <template v-if="showWorkspaceHeader" #left>
          ...
          <template #right>
            <UserNavigation v-if="!shouldShowChat" />
          </template>
        </UDashboardNavbar>
      </template>
